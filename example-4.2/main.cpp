...
// Defining variables related to the rod, SI units
const double areaCrossSec{1}; // We assume a unit area here
const double lengthRod{0.02};
const double T_A{100};
const double T_B{200};
const double k{0.5};
const double q{std::pow(10, 6)};
...
int main()
{
    ...
    */
    ...
    A.at(0, 0) = 3 * coeff;
    A.at(0, 1) = -coeff;
    B.at(0) = q * areaCrossSec * deltaX + (250 * T_A);

    // Right boundary
    A.at(A.n_rows - 1, A.n_cols - 1) = 3 * coeff;
    A.at(A.n_rows - 1, A.n_cols - 2) = -coeff;
    B.at(B.n_rows - 1) = q * areaCrossSec * deltaX + (250 * T_B);
    ...
    */
    ...
    // std::cout << "The B amtrix is - " << B << std::endl;
